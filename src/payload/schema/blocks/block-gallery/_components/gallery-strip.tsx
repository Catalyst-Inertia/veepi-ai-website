  ...
  return (
    ...
    >
      {Array.from({ length: TILE_REPEAT * COLUMN_PATTERN.length }).map(
        (_, colIndex) => {
          const columnCards = COLUMN_PATTERN[colIndex % COLUMN_PATTERN.length]
            ...
            >
              {columnCards.map((aspectClass, i) => {
                ...
                for (let c = 0; c < colIndex; c++) {
                  slotIndex += COLUMN_PATTERN[c % COLUMN_PATTERN.length].length
                }
                slotIndex += i
                ...
                return (
                  <article
                    ...
                    className={`group relative w-full overflow-hidden rounded-2xl cursor-pointer ${aspectClass}`}
                  >
